...
public class HazelcastJdbcResultSet extends JdbcResultSetAdapter {
    ...
    public ResultSetMetaData getMetaData() throws SQLException {
        ...
        for (int i = 1; i <= columnCount; i++) {
            String columnName = cursor.getColumnName(i - 1);
            metadata.setColumnName(i, columnName);
            metadata.setColumnLabel(i, columnName);

            SqlColumnType type = cursor.getColumnType(i - 1);
            if (type == SqlColumnType.BOOLEAN) {
                metadata.setColumnType(i, JDBCType.BOOLEAN.getVendorTypeNumber());
            } else if (type == SqlColumnType.INTEGER) {
                metadata.setColumnType(i, JDBCType.INTEGER.getVendorTypeNumber());
            } else if (type == SqlColumnType.BIGINT) {
                metadata.setColumnType(i, JDBCType.BIGINT.getVendorTypeNumber());
            } else {
                metadata.setColumnType(i, JDBCType.VARCHAR.getVendorTypeNumber());
            }
        }
